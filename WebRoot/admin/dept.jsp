...
<c:if test="${fn:contains(sessionInfo.resourceList, '/deptController/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/deptController/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/deptController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前部门？', function(b) {
			if (b) {
				...
				$.post('${pageContext.request.contextPath}/deptController/delete', {
					id : id
				}, function(result) {
					if (result.success) {
		...
		if (rows.length > 0) {
			parent.$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
				if (r) {
					...
					$.getJSON('${pageContext.request.contextPath}/deptController/batchDelete', {
						ids : ids.join(',')
					}, function(result) {
						if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 500,
			height : 300,
			href : '${pageContext.request.contextPath}/deptController/editPage?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
...
<body>
		...
		<div data-options="region:'north',title:'查询条件',border:false" style="height: 160px; overflow: hidden;">
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="display: none;">
					<tr>
						<th>部门中文名称</th>
						<td><input name="cnname" placeholder="可以模糊查询中文名" class="span2" /></td>
					</tr>
					<tr>
						<th>部门英文名称</th>
						<td><input name="enname" placeholder="可以模糊查询英文名" class="span2" /></td>
					</tr>
					<tr>
						<th>创建时间</th>
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/deptController/addPage')}">
			<a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'pencil_add'">添加</a>
		</c:if>
		<c:if test="${fn:contains(sessionInfo.resourceList, '/deptController/batchDelete')}">
